...
// ── Fast schedule update (uses existing userdata, no GitHub) ──
app.post('/update-sched', async (req, res) => {
  ...
  try {
    const schedJson = JSON.stringify(currentSched || {}, null, 2);
    const message = await client.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 8000,
      messages: [{ role: 'user', content: 'Edit this weekly schedule JSON. Current:\n' + schedJson + '\n\nInstructions (apply ALL):\n' + instructions + '\n\nReturn ONLY the updated { } JSON object. No markdown, no explanation.' }]
    });
    let newSched = message.content[0].text.trim();
    const _fStart = newSched.indexOf('{'), _fEnd = newSched.lastIndexOf('}');
    if(_fStart > 0) newSched = newSched.slice(_fStart, _fEnd + 1);
    if (!newSched.startsWith('{') || !newSched.includes('Monday'))
      return res.status(500).json({ error: 'AI returned invalid schedule' });
    const parsed = JSON.parse(newSched);
    // Save using existing setUserData helper
    await setUserData('user_schedule', JSON.stringify(parsed));
